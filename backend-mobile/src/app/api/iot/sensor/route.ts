...
// 🚨 PERBAIKAN: Ditambahkan kata kunci kelas baru (penebangan_liar & pertambangan_ilegal)
const DAFTAR_ANCAMAN_LEGACY = [
  'penebangan_liar',
  'pertambangan_ilegal',
  'penebangan',
  'pertambangan',
...
function isLabelAncaman(label: string): boolean {
  ...
  if (normalized === LABEL_ANCAMAN_BINARY.toLowerCase() || normalized === 'positif' || normalized.includes('ancaman')) return true;
  return DAFTAR_ANCAMAN_LEGACY.some((threat) => normalized.includes(threat));
}

// 🌐 MENGAMBIL BASE URL DARI `.env` MURNI
