...
// 🚀 KONFIGURASI TERPUSAT DOMAIN PRODUCTION AI SERVER
const String kFallbackAiServerBaseUrl = "https://api-rimbarest.lab-ilkom.my.id";
...
class _AudioEventListState extends State<AudioEventList> {
  ...
  // PENGHASIL SUARA SINTETIS FALLBACK (Jika berkas di server benar-benar tidak ada)
  ...
  /// 🚀 RESOLVER URL PRODUCTION:
  /// Mengarahkan file audio ke endpoint /static-audio/ pada domain AI Server Production
  /// jika URL mengarah ke Supabase, path lokal, IP lokal, atau "localhost"/"127.0.0.1"
  String _resolveAudioUrl(String rawUrl) {
    final trimmed = rawUrl.trim();
    final lower = trimmed.toLowerCase();

    final bool isUntrustedHost = lower.contains("localhost") ||
        lower.contains("127.0.0.1") ||
        lower.contains("10.244.");
  ...
  Future<void> _togglePlayAudio(String? url, int index, String label) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint(
            "🚨 Masalah koneksi jaringan ke AI Server ($secureUrl): $networkError");
      }
      ...
      if (targetBytes != null && targetBytes.isNotEmpty) {
        ...
        if (mounted) {
          _showSnackBar(
              "🔔 Rekaman asli gagal dimuat dari server, memutar audio simulasi",
              AppTheme.danger);
        }
      }
