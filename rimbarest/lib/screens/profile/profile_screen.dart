...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> saveSetting({
    ...
    try {
      ...
      // 🚀 2. SINKRONISASI AKTIF KE TABEL DeviceToken VIA API PRODUCTION
      if (notif != null) {
        try {
          String? fcmToken = await FirebaseMessaging.instance.getToken();
          if (fcmToken != null) {
            final url = Uri.parse('${ApiConfig.baseUrl}/mobile/user/token');
            await http.post(
              url,
              headers: {
      ...
      debugPrint("🚨 Gagal menyimpan pengaturan ke server: $e");
